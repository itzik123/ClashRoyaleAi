...
from python_ai.rewards.elixir_shaping import W_SOLVENCY, solvency_shaping
...
def train_ppo():
    ...
    # Absolute path, not "train_selfplay.py": the two pipelines live in
    # python_ai/trainers/ while the run's cwd is python_ai/ (where the
    # checkpoints are), so a bare filename would not resolve.
    subprocess.Popen(
        [sys.executable, "-u",
