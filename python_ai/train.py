...
def compute_shaping(obs, prev_obs, w_bldg=W_BLDG, w_troops=W_TROOPS):
    ...
    d_ally_troops = obs_spatial[:, 0].sum(axis=(1, 2)) - prev_spatial[:, 0].sum(axis=(1, 2))
    d_enemy_troops = obs_spatial[:, 1].sum(axis=(1, 2)) - prev_spatial[:, 1].sum(axis=(1, 2))
    d_ally_bldg = obs_spatial[:, 2].sum(axis=(1, 2)) - prev_spatial[:, 2].sum(axis=(1, 2))
    d_enemy_bldg = obs_spatial[:, 3].sum(axis=(1, 2)) - prev_spatial[:, 3].sum(axis=(1, 2))
...
def train_ppo():
    ...
    ppo_epochs = 4         # How many times to reuse each rollout
        ...
        for step in range(update_timestep):
            ...
            for i, done in enumerate(dones):
                if done:
                    ...
                    win_history.append(1.0 if step_rewards[i] > 0.5 else 0.0)

                    if episodes_completed % 10 == 0:
                        avg_reward = np.mean(reward_history)
                        avg_shaping = np.mean(shaping_history)
                        win_rate = np.mean(win_history)
                        print(f"Episodes: {episodes_completed} | Avg(50): {avg_reward:.2f} | Shaping(50): {avg_shaping:.2f} | WinRate(100): {win_rate:.2f} | Stage: {curriculum_stage} | Entropy: {current_entropy_coef:.4f}")
                        ...
                        writer.add_scalar("Training/Win_Rate_100", win_rate, episodes_completed)
                    ...
                    # --- Curriculum advancement: escalate opponent elixir once win-rate is stable and high ---
                    stage_threshold = CURRICULUM_STAGES[curriculum_stage]["win_rate_threshold"]
                    if (stage_threshold is not None
                            and len(win_history) == win_history.maxlen
                            and np.mean(win_history) >= stage_threshold
        ...
        for epoch in range(ppo_epochs):
            ...
            for mb_start in range(0, num_envs, mb_size):
                ...
                for t in range(update_timestep):
                    logits_t, mean_t, log_std_t, value_t, (rhx, rcx) = net(obs_seq[t, mb_env_t], (rhx, rcx))
