"""Guards for serving the mined human placement prior into training. Three would
be silent if broken:

  - the default: the prior ships off, so the A/B that justifies it runs both arms from byte-identical code;
  - precedence: mined from the real game, whose physics this engine does not reproduce, it fills gaps and never overrides a rule measured against this engine;
  - mutual exclusivity in the loss: `coverage_terms` gives a row KL or an entropy bonus, never both, and a non-1.0 weight in `coverage_has` must not break that.
"""
import numpy as np
import pytest
import torch
...
# The deck cards no hand-written rule reaches: the gap the prior fills,
...
def _fresh_obs():
    """A board with a live threat: the prior declines on a quiet board by design,
    so a quiet fixture would make every "the prior speaks" case vacuous.
    """
    env = CE(list(DEFAULT_DECK), list(DEFAULT_DECK), max_ticks=3600)
    env.seed(1)
    env.reset()
...
def test_the_prior_ships_OFF():
    """The A/B control arm is the default and costs nothing."""
...
def test_the_five_uncovered_cards_really_have_no_rule():
    """Pins the gap the prior fills; if a rule is added for one of these, the
    precedence test becomes the live one.
    """
...
def test_the_handwritten_rule_wins_where_it_speaks(prior_on, monkeypatch):
    """A rule measured against this engine outranks a prior mined from a game it
    does not reproduce.
    """
...
def test_weights_are_1_for_a_rule_and_the_coef_for_the_prior(prior_on):
    ...
    # A rule row carries weight 1.0; force the rule to speak so the branch is
...
def test_a_weighted_row_gets_KL_and_NOT_an_entropy_bonus():
    """The invariant `coverage_terms` holds: an entropy mask of `1 - has_target`
    would give a 0.1-weighted row 90% of a spread-out bonus while its KL says
    where to go.
    """
    ...
    # One row has a target either way, so the entropy half sees the same other
    # row: identical entropy fraction.
...
def test_a_stale_prior_is_refused_rather_than_reinterpreted(tmp_path,
                                                            monkeypatch):
    """An arena change must raise, not silently serve cells for a different board.
    """
...
def test_the_OFF_arm_is_the_OLD_behaviour_exactly():
    """The A/B control must be the old code exactly: while disabled,
    `prior_cards()` must be empty, or `slot_weights_for` would weight every
    deck card and change the control arm's coverage draw.
    """
    assert not human_prior.enabled()
...
def test_the_prior_declines_on_a_quiet_board(prior_on):
    """Every target source declines on an empty board, "or it teaches a constant".
    """
    env = CE(list(DEFAULT_DECK), list(DEFAULT_DECK), max_ticks=3600)
    env.seed(0)
    env.reset()
...
def test_the_prior_still_speaks_once_there_is_a_threat(prior_on):
    """The gate is not so wide that the prior never trains."""
    env = CE(list(DEFAULT_DECK), list(DEFAULT_DECK), max_ticks=3600)
    env.seed(5)
    env.reset()
    for _ in range(15):
