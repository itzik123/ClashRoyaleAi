"""PPOConfig: the hyperparameters, and the invariants they must satisfy."""
import dataclasses
...
def test_the_documented_defaults_are_the_ones_every_measurement_used():
    cfg = PPOConfig()
    # `1/(1-gamma)` is the horizon in decisions, and a full match is 360; the
    # relationship is pinned by tests/test_reward_horizon_invariant.py.
    assert cfg.gamma == 0.999
    assert cfg.gae_lambda == 0.9
    assert cfg.eps_clip == 0.2
    assert cfg.lr == 3e-4
    assert cfg.update_timestep == 500
    # The credit horizon must outlast a card rotation (see
...
def test_update_timestep_must_divide_evenly_into_bptt_chunks():
    """A partial trailing chunk would train on a shorter segment, silently
    weighting the end of each rollout differently.
    """
...
def test_segments_per_rollout_is_chunks_times_envs():
    ...
    is 10 segments, bounded at >= 8 by test_bptt_credit_horizon.py.
    """
...
def test_the_config_is_frozen():
    """A trainer that mutated its config mid-run would make a checkpoint's episode
    count meaningless as a description of what produced it.
    """
    cfg = PPOConfig()
...
def test_the_gamma_used_for_shaping_is_the_same_one_GAE_uses():
    """Potential-based shaping is policy-invariant only if the gamma in
    gamma*Phi(s') - Phi(s) is the one the returns discount with.
    """
    cfg = PPOConfig()
    import inspect
    ...
    # ...and `compute_shaping` must have no default gamma to fall back to
    # (test_shaping_gamma_is_not_a_second_copy.py pins it too). A convenience
    # default would reopen silent drift without breaking the assertion above.
...
def test_the_two_entropy_configs_differ_only_where_they_are_meant_to():
    ...
    The four intended differences: the placement anneal start, the placement
    gain, the per-update step cap, and the placement ceiling.
    """
