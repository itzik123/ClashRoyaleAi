"""The trainee's deck: one definition (`python_ai.deck`), settable through
`CLASH_DECK` by id or name, and a bad deck fails at parse time with the
engine's own reason.
"""
import pytest
...
def test_an_evolution_is_named_with_an_evo_prefix():
    """Evolutions reuse the base card's name (id 1 and id 128 are both "Archers"),
    so a bare name means the base card and an Evolution is asked for
    explicitly.
    """
