...
These pin the mechanism on real engine boards.
"""
import numpy as np
import pytest
...
def test_fireball_is_aimed_exactly_as_before():
    """Control: Fireball keeps its geometry."""
...
def test_a_roller_keeps_the_disc_it_was_aimed_with_before():
    """The Log's value is a corridor that no disc describes, and the probe
    declines it; it keeps the disc it had.
    """
...
def test_suppression_uses_the_cards_own_disc():
    """The second candidate is suppressed with the cast spell's own disc: Poison
    (3.5) suppresses more than Fireball (2.5).
    """
...
def test_the_rules_gate_fires_a_zap_on_a_skeleton_clump():
    """243 HP of Skeletons is a full-damage Zap; Fireball's 689 bar would hold it
    back.
    """
...
def test_the_rules_gate_holds_a_rocket_back_from_a_lone_musketeer():
    ...
    689 bar would let it through, the Rocket's own 1485 does not.
    """
...
def test_the_rules_gate_is_unchanged_for_fireball_on_both_boards():
    """Control for the two above: Fireball decides as before."""
