...
// Excludes Towers on purpose ("enemy troop", matching both sourced Hero
// abilities' own wording). Returns nullptr if nothing eligible is found.
inline std::shared_ptr<Entity> findHpExtremeEnemy(Board& board, const Vector2D& origin,
    ...
    for (const auto& entity : board.getEntities()) {
        if (entity->team == myTeam || !entity->isAlive() || !entity->isTargetable() || entity->isTower()) continue;
